...
class SwinTransformerElement(nn.Module):
    ...
    def __init__(self, embed_dim, input_resolution, num_heads, window_size=8, shift_size=0,
...
class FusionTransformer(nn.Module):
    ...
    def __init__(self, embed_dim, input_resolution, num_heads, window_size=7, shift_size=0, channel_scale = 2,
                 mlp_ratio=4., qkv_bias=True, qk_scale=None, drop=0., attn_drop=0., drop_path=0.,
                 act_layer=nn.GELU, norm_layer=nn.LayerNorm):
        super().__init__()
        self.dim = embed_dim
...
class BasicLayer(nn.Module):
    ...
    def __init__(self, embed_dim, input_resolution, num_heads, window_size, depth=2,
    ...
    def forward(self, x, y):  ### fpr depth = 2
...
class SelfCrossBlocks(nn.Module):
    ...
    Args:
        ...
        drop_path (float | tuple[float], optional): Stochastic depth rate. Default: 0.0
        norm_layer (nn.Module, optional): Normalization layer. Default: nn.LayerNorm
        n_SC: Number of self- and cross-attention blocks
    """

    def __init__(self, embed_dim, input_resolution, num_heads, window_size, depth=2,
                 mlp_ratio=4., qkv_bias=True, qk_scale=None, drop=0., attn_drop=0.,
                 drop_path=0., norm_layer=nn.LayerNorm, n_SC=2):
...
class ViT(nn.Module):
    ...
    Args:
        dim (int): Number of input channels.
        input_resolution (tuple[int]): Input resolution.
        depth (int): Number of blocks in each self or cross ViT.
        num_heads (int): Number of attention heads.
        window_size (int): Local window size.
        mlp_ratio (float): Ratio of mlp hidden dim to embedding dim.
        qkv_bias (bool, optional): If True, add a learnable bias to query, key, value. Default: True
    ...
    def __init__(self, dim, img_size, num_heads, window_size, depth=2, mlp_ratio=4., 
                 qkv_bias=True, qk_scale=None, drop=0., attn_drop=0., patch_norm=True,
                 drop_path=0., norm_layer=nn.LayerNorm, patch_size=4, n_SC=2, embed_dim=96, ds_ref=True):
