...
class fD2RT(nn.Module):
    def __init__(self, coils, img_size, num_heads, window_size, depth=2, patch_size = None,
                 mlp_ratio=4., qkv_bias=True, qk_scale=None, drop=0., attn_drop=0.,
                 drop_path=0., norm_layer=nn.LayerNorm, n_SC=2, num_recurrent=5, embed_dim=96, sens_chans=8,
                 sens_steps=4, mask_center=True, ds_ref=True, SR_scale=1, is_Unet=True):
        super().__init__()
        ...
        self.recurrent = nn.ModuleList([DDRB(coils_all=coils, img_size=img_size, num_heads=num_heads, window_size=window_size, depth=depth, 
    ...
    def forward(self, Ref_Kspace_f, Target_Kspace_u, mask, num_low_frequencies):
        ...
        for idx, DDRB_ in enumerate(self.recurrent):
            ...
            Target_img_f = DDRB_(Ref_img, Ref_Kspace_f, Target_Kspace_u, Target_img_f, mask, sens_maps_updated)
            rec.append(Target_img_f)
         
        return rec, utils.rss(Target_img_f), sens_maps_updated, Target_img_f, SMs
